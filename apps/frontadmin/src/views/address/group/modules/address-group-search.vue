...
defineOptions({ name: 'AddressGroupSearch' });
...
const model = defineModel<Api.Address.AddressGroupSearchParams>('model', { required: true });
            ...
            <ElCol :lg="6" :md="8" :sm="12">
              <ElFormItem :label="$t('page.address.common.status')" prop="status">
                <ElSelect
                  ...
                  :empty-values="['']"
                  value-on-clear=""
                  :placeholder="$t('page.address.common.status')"
                >
                  <ElOption
                    v-for="(item, idx) in translateOptions(addressGroupStatusOptions)"
                    :key="idx"
                    :label="item.label"
                    :value="item.value"
                  ></ElOption>
